...
<a href="{{ route('sponsored.page') }}">
    ...
    {{ __('index.add_sponsored_ad') }}

</a>

</a-menu-item>

<a-menu-item key="account.order.index"   @if(Route::currentRouteName() == 'order.seller') class="activeb" @endif>

<a href="{{ route('order.seller') }}">

<span class="fa fa-gift"></span>  {{__('index.orders')}} علي اعلاناتك &nbsp; <span class="badge badge-pill">{{count(Auth::user()->order)}}</span>
...
<span class="fa fa-shopping-bag"></span>  {{__('index.shop_info')}}
...
<a href="{{route('shops2', auth()->user()->slug)}}" >

<span class="fa fa-shopping-bag"></span>  {{__('index.your_shops')}}
...
<a-menu-item key="account.searches.index" @if(Route::currentRouteName() == 'search.saved') class="activeb" @endif>

<a href="{{ route('search.saved') }}">

<span class="fa fa-floppy-o"></span>    {{__('index.saved_search')}}
