...
@section('content')
                ...
                <ul class="breadcrumb">
                    ...
                    <li><a href="{{route('category.sub', $ad_attr[0]->product->categories->id ?? '')}}">{{$ad_attr[0]->product->categories->name ?? ''}}</a>
                    </li>
                    <li><a href="#">{{$ad_attr[0]->product->subcategory->name ?? ''}}</a>
                    </li>
                </ul>
     ...
     <!-- <button class="btn btn-primary secondNavButtons" style="
            ...
            "><a class=""  style="color: white;text-decoration: none;"> <i class="fa fa-search" aria-hidden="true" style="background-color: transparent;height: 0;line-height: 0;padding-top: 10px;color:black;"></i> search</a> 
                ...
                <div class="col-md-9">
                    ...
                    @foreach($ad_attr as $p)
                        <div class="col-md-4">
                      
                      <a href="{{route('product.details', $p->product->slug)}}">
                      ...
                      <img src="{{$p->product->main_image_url}}" alt="Image Alternative text" title="{{$p->product->name}}" style="height: 200px;" />
                      </header>
                      <div class="product-inner">
                      <ul class="icon-group icon-list-rating" title="{{ number_format($p->product->averageRating)}}/5 rating">
                          ...
                          <h5 class="product-title">{{$p->product->name}}</h5>
                                      <div class="product-desciption"><span class="item-location"><i class="fa fa-map-marker"></i> {{$p->product->city->city ?? ''}}</span>   <i class="fa fa-clock-o"></i> <span>{{date_format($p->product->created_at,'d M Y')}}</span>
                                      <br> 
                                      </div>
                          ...
                          <div class="product-meta">
                              <ul class="product-price-list">
                                  <li><span class="product-price">{{$p->product->price}} {{$p->product->country->currency}}</span>
                                  </li>
                              </ul>
