...
use AvoRed\Framework\Database\Models\SavedSearch;
use Auth;
...
$agent =new Agent();
        ...
        if($request->type == 'free'){
          ...
          // if type = sponsored ads
            ...
            if($request->category != 'null' && $request->name = 'null'){
                    ...
                    $product = Product::where([['category_id',$request->category],['approved','1'], ['country_id', session()->get('my_country')]])->orderBy('sponsored','DESC')->orderBy('id','DESC')->paginate(9);
            }
                  else  if($request->category !='null' && $request->name != 'null' ){
                        //search by name and category and
                        $pro= Product::Query()->ByName($request->name)
                        ->where([['category_id',$request->category],['approved','1'], ['country_id', session()->get('my_country')]])->orderBy('sponsored','DESC')->orderBy('id','DESC')->paginate(9);
                        $serv = Service::Query()->ByName($request->name)->where([['approved','1']])->get();
                        // $product= $pro->merge($serv); // when search in product and services********
                        $product = $pro;

                    }else{
                        $pro= Product::Query()->ByName($request->name)->where([['approved','1'], ['country_id', session()->get('my_country')]])->orderBy('sponsored','DESC')->orderBy('id','DESC')->paginate(9);
                    $serv = Service::Query()->ByName($request->name)->where([['approved','1']])->get();
                    // $product= $pro->merge($serv); // when search in product and services********
                    $product = $pro;
                    session::put('saved_search', $request->name);
                    }
      ...
      //  dd($product);
if($agent->isMobile()){
    return view('blades_mobile.search_mobile',compact('page_title','product','productonly'));
...
//   dd($q) ;
}]])->orderBy($column , $direction)->orderBy('id','DESC')->paginate(9);
...
if( $agent->isMobile()){
    return view('blades_mobile.search_mobile',compact('page_title','product'));
...
public function getSavedSearch(){
    $agent= new Agent();
    $search = SavedSearch::where('user_id',auth()->user()->id)->orderBy('created_at','DESC')->limit(5)->get();
    if($agent->isMobile()){
        return view('blades_mobile.savedSearch_mobile',compact('search'));
    }
    return view('blades.savedSearch',compact('search'));
}

public function saveSearch(Request $request){
    //dd($request);
    // dd(session::get('saved_search'));
    if(session::get('saved_search') != null){
        $search = new SavedSearch;
        $search->user_id=auth()->user()->id;
        $search->search = session::get('saved_search');
        $search->save();
        Session::flash('message', 'Search Saved Successfuly');
